...
export function decodeEntities(input: string): string {
  ...
  return input.replace(/&(#[xX][0-9a-fA-F]+|#\d+|[a-zA-Z][a-zA-Z0-9]{1,7});/g, (entity, body: string) => {
...
export function extractPage(html: string, options: ExtractOptions): ExtractedPage {
  ...
  let anchor: { href: string } | undefined
  let anchorBuf = ''
  ...
  for (const token of html.matchAll(tokenRe)) {
    ...
    if (chunk.startsWith('<')) {
      ...
      if (tagMatch?.[1] === undefined) continue // <!doctype> and friends
      ...
      if (!closing) {
        ...
        if (headingLevel !== undefined && attributes !== undefined) {
          flushAnchor()
          ensureBreak()
          if (markdown) out += '#'.repeat(headingLevel) + ' '
          continue
        }
  ...
  if (truncated) content = content.slice(0, options.maxChars)
