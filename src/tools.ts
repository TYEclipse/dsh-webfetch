...
import { extractTables, type ExtractedTable } from './table.ts'
import { fetchFeed, fetchPage, resolveHref, type FetchedPage, type FetchConfig } from './fetch.ts'
import { parseFeed, truncateText } from './feed.ts'
import { probeUrl, type HeaderProbe, type RedirectHop } from './head.ts'
...
function renderFeed(value: unknown): string {
  const result = value as { feedTitle: string; finalUrl: string; entryCount: number; truncated: boolean; entries: FeedItem[] }
  const lines = [
  ...
  for (const [index, entry] of result.entries.entries()) {
    ...
    if (entry.published !== undefined) lines.push(`   published: ${entry.published}`)
    if (entry.author !== undefined) lines.push(`   author: ${entry.author}`)
    if (entry.summary !== undefined) lines.push(`   ${entry.summary}`)
    if (entry.content !== undefined) lines.push(`   ${entry.content}`)
  }
  if (result.truncated) lines.push('[feed truncated — response body exceeded the size cap]')
  return lines.join('\n')
}

/** Compact text renderer for web_headers results. */
function renderHeaders(value: unknown): string {
  const result = value as HeaderProbe
  const lines = [`HTTP ${result.status} ${result.statusText} — ${result.method} ${result.url}`]
  if (result.url !== result.finalUrl) lines.push(`final URL: ${result.finalUrl}`)
...
function renderTable(value: unknown): string {
  const result = value as { finalUrl: string; tableCount: number; totalTables: number; truncated: boolean; tables: ExtractedTable[] }
  if (result.tableCount === 0) return `no tables found on ${result.finalUrl}`
  ...
  for (const table of result.tables) {
    ...
    if (table.header.length > 0) lines.push(`  [header] ${table.header.join(' | ')}`)
    for (const row of table.rows) lines.push(`  ${row.join(' | ')}`)
  }
  if (result.truncated) lines.push('[output capped — raise maxRows/maxTables or target one table with the `table` argument]')
  return lines.join('\n')
}
...
export function buildWebfetchTools(config: ResolvedConfig): ToolSet {
  ...
  const web_feed = defineTool({
    ...
    output: {
      ...
      render: (_args: { url: string }, value: unknown) => [{ type: 'text', text: renderFeed(value) }],
    },
    async execute(args: { url: string; maxItems?: number; includeContent?: boolean }) {
      ...
      const entries: FeedItem[] = feed.entries.slice(0, maxItems).map((entry) => {
        const item: FeedItem = { title: entry.title, url: resolveHref(entry.url, fetched.finalUrl) }
        if (entry.published !== '') item.published = entry.published
        if (entry.author !== '') item.author = entry.author
        if (entry.summary !== '') item.summary = truncateText(entry.summary, 500)
        if (args.includeContent ?? false) {
          const full = entry.content !== '' ? entry.content : entry.summary
          if (full !== '') item.content = truncateText(full, 2_000)
        }
        return item
      })
      return {
  ...
  const web_headers = defineTool({
    ...
    output: {
      ...
      render: (_args: { url: string }, value: unknown) => [{ type: 'text', text: renderHeaders(value) }],
    },
    async execute(args: { url: string; method?: string; followRedirects?: boolean }) {
      const method = args.method === 'GET' ? 'GET' : 'HEAD'
      const result: HeaderProbe = await probeUrl(args.url, config, method, args.followRedirects ?? true)
      return {
        url: result.url,
        ...
        redirects: result.redirects.map((hop: RedirectHop) => ({ url: hop.url, status: hop.status, location: hop.location })),
      }
    },
  })
  ...
  const web_table = defineTool({
    ...
    output: {
      ...
      render: (_args: { url: string }, value: unknown) => [{ type: 'text', text: renderTable(value) }],
    },
    async execute(args: { url: string; table?: number; maxTables?: number; maxRows?: number }) {
      ...
      let tables: ExtractedTable[] = extracted.tables
